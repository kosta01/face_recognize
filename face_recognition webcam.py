...
class Login:
    def __init__(self, root):
        ...
        submit=Button(frame_login, cursor="hand2", command= self.check_function, text= "Kyç",bd=0, font=("Goudy old style", 15), bg="#6162FF", fg="white").place(x=90, y=320, width=180, height=40)
    ...
    def check_function(self):
        if self.username.get()=="" or self.password.get()=="":
           messagebox.showerror("Error", "Kerkohen te plotesohen te gjitha fushat!", parent=self.root)
            ...
            R_image = fr.load_image_file("faces\R.jpeg")
            R_face_encoding = fr.face_encodings(R_image)[0]
            known_face_encondings = [R_face_encoding]
            known_face_names = ["rod"]
            B_image = fr.load_image_file("faces\B.jpeg")
            B_face_encoding = fr.face_encodings(B_image)[0]
            known_face_encondings = [B_face_encoding]
            known_face_names = ["Brisejda"]
            known_face_encondings=[kostandina_face_encoding,H_face_encoding,R_face_encoding, Oliverta_face_encoding, B_face_encoding]
            known_face_names=["Kostandina Qirjazi",
            "Harlidis Miloti", "Rod Kruja", "Oliverta Murraj", "Brisejda"]
            face_locations=[]
            face_encoding=[]
            face_names=[]
...
# Release handle to the webcam, Closes webcam
            ...
            sheet1.write(1, 0,known_face_names[best_match_index])
            book.save("listprezenca.xls")
            open("Listprezenca.xls")
